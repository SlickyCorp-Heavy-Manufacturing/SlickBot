...
export class Covid {
  ...
  public static async wiVaccine(msg: Message): Promise<void> {
    ...
    let complete: FileResult;
    try {
      first = await file({ prefix: 'first-', postfix: '.png' });
      complete = await file({ prefix: 'complete', postfix: '.png' });
      await Promise.all([
        got.get(
        ...
        ).then((response) => fs.writeFileSync(first.path, response.body)),
        got.get(
          'https://website-snapshot.krischeonline.com/vaccine-wi-complete',
          {
            headers: {
              API_KEY: process.env.SNAPSHOT_API_TOKEN,
            },
        ...
        ).then((response) => fs.writeFileSync(complete.path, response.body)),
      ]);
      await msg.channel.send({ files: [first.path, complete.path] });
    } finally {
      if (first) {
        fs.unlinkSync(first.path);
      }
      if (complete) {
        fs.unlinkSync(complete.path);
      }
    }
  }
